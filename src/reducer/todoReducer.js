...
import {CREATE_TODO, DELETE_TODO, FETCH_TODO, TOGGLE_TODO, UPDATE_COMMENT, UPDATE_TODO} from '../actions/todoAction'
...
const updateCommentReducer = (state, action) => {
    return state.map((todo) => {
        if (todo.id === action.id) {
            return {...todo, updatedTime: Date.now(), comments: todo.comments.concat(action.comment)}
        } else {
            return todo
        }
    })
